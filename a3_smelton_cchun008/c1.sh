...
bash genPad.sh ":smelton:->:cchun008:" M1
...
openssl dgst -mac HMAC -macopt key:":smelton:->:cchun008:" -md4 M1 | sed 's/^.*= //' > M1.hmac 
echo -n "`cat M1.hmac`" > M1.hmac

base64 -d M1Sig.base64 > M1Sig

openssl rsautl -verify -pubin  -inkey smeltonPublicKey.pem -in M1Sig -out  M1.hmac
...
# echo -n ":C:->:Y:" > P2
bash genPad.sh ":cchun008:->:smelton:" M2
...
openssl dgst -mac HMAC -macopt key:":cchun008:->:smelton:" -md4 M2 | sed 's/^.*= //' > M2.hmac 
