...
bash genPad.sh ":pvargas:->:cchun008:" M1
...
openssl dgst -mac HMAC -macopt key:":pvargas:->:cchun008:" -md4 M1 | sed 's/^.*= //' > M1.hmac 
echo -n "`cat M1.hmac`" > M1.hmac

base64 -d M1Sig.base64 > M1Sig

openssl rsautl -verify -pubin  -inkey pvargasPublicKey.pem -in M1Sig -out  M1.hmac

# 3.     Edit a reply M2 that have of at least 20 new byes followed by the received plain message M1.

echo -n "Hello my friend, this is a reply from Chak Pong" >> M2
echo "" >> M2
date >> M2
echo "" >> M2
...
# echo -n ":C:->:Y:" > P2
bash genPad.sh ":cchun008:->:pvargas:" M2
...
openssl dgst -mac HMAC -macopt key:":cchun008:->:pvargas:" -md4 M2 | sed 's/^.*= //' > M2.hmac 
echo -n "`cat M2.hmac`" > M2.hmac
openssl rsautl -sign -inkey cchun008PrivateKey.pem -in M2.hmac | base64 > M2Sig.base64 
